...
export default function Programs() {
  ...
  const [currentUserRole, setCurrentUserRole] = useState<SafeUserRole>("Unknown");
  ...
  const canCreate = currentUserRole === "SuperAdmin" || currentUserRole === "Admin";
  ...
  const handleAddProgram = async (e: React.FormEvent) => {
    ...
    if (!canCreate) {
      toast.error("You do not have permission to add program");
      return;
    }
  ...
  const handleEditProgram = async (program: Program) => {
    if (!canEdit) {
      toast.error("You do not have permission to edit program");
      return;
    }
  ...
  const confirmDelete = async () => {
    ...
    if (!canDelete) {
      toast.error("You do not have permission to delete program");
      return;
    }
  ...
  const handleViewProgram = (p: Program) => {
            ...
            <thead className="bg-soft sticky top-0">
              <tr>
                {["Program Name", "Level / Type", "Status", "Actions"].map((h) => (
                  <th key={h} className="px-6 py-3 text-left text-body font-medium">
                    {h}
                  </th>
                ))}
              </tr>
        ...
        {!loading && programs.length === 0 && (
          <div className="p-12 text-center">
            ...
            <p className="text-body">
              No programs found. {canCreate ? "Add your first program to get started." : ""}
            </p>
          </div>
        )}
